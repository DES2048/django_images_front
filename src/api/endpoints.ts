...
function encodePar(par: string): string {
...
const endpoints: APIEndpoints = {
  ...
  deleteImage(gallery: string, imgName: string) {
    return new URL(`/delete-image/${gallery}/${encodePar(imgName)}`, API_BASE_URL);
  },
  markImage(gallery: string, imgName: string) {
    return new URL(`/galleries/${gallery}/images/${encodePar(imgName)}/mark`, API_BASE_URL);
  },
  unmarkImage(gallery: string, imgName: string) {
    return new URL(`/galleries/${gallery}/images/${encodePar(imgName)}/unmark`, API_BASE_URL);
  },
  ...
  imageTags(gallery: string, imgName: string) {
    return new URL(`/galleries/${gallery}/images/${encodePar(imgName)}/tags`, API_BASE_URL);
  },
